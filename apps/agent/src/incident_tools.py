...
# Secondary chart per incident type — correlated diagnostic metric unique to each failure mode
SECONDARY_CHART_CONFIGS: dict = {
    "data_drift": {
        "metric": "feature_importance_shift",
        "title": "Feature Importance Shift (KL Divergence)",
        "unit": "nats",
        "baseline": 0.04,
        "threshold": 0.15,
        "spike_mag": 0.22,
    ...
    "feature_skew": {
        "metric": "skew_divergence",
        "title": "Training vs Serving Feature Divergence",
        "unit": "\u0394\u03c3",
        "baseline": 0.08,
        "threshold": 0.30,
        "spike_mag": 0.41,
...
def build_charts_for_incident(
    ...
    """Build 2 ChartSpecs per incident type.
    Chart 1: Primary KPI for this failure mode.
    Chart 2: Correlated diagnostic metric semantically specific to the incident.
    """
    cfg = CHART_CONFIGS.get(incident_type, CHART_CONFIGS["data_drift"])
    sec = SECONDARY_CHART_CONFIGS.get(incident_type, SECONDARY_CHART_CONFIGS["data_drift"])

    # Primary chart — canonical KPI
    ...
    # Secondary chart — incident-specific diagnostic
    sec_data, sec_anomalies = generate_timeseries(
        metric=sec["metric"],
        window_minutes=60,
        points=60,
        baseline=sec["baseline"],
        spike_at=sec.get("spike_at", 0.75),
        spike_magnitude=sec["spike_mag"],
    ...
    return [
        ...
        {
            "id": str(uuid.uuid4()),
            "type": sec.get("chart_type", "line"),
            "title": f"{sec['title']} \u2014 {model_name}",
            "metric": sec["metric"],
            "unit": sec["unit"],
            "data": sec_data,
            "threshold": sec["threshold"],
            "anomaly_windows": sec_anomalies,
            "color": sec["color"],
        },
    ]
