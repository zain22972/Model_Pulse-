...
interface WebhookTriggerPanelProps {
  onFire?: (alertType: string, model: string) => void;
}

export function WebhookTriggerPanel({ onFire }: WebhookTriggerPanelProps) {
  ...
  const fire = async (alertType: string, model: string) => {
    ...
    // If onFire is provided (Approach A), call it.
    // This injects the alert into CopilotKit.
    if (onFire) {
      onFire(alertType, model);
    }

    try {
      // We still POST to the webhook so the IncidentFeed (SSE) sees it
